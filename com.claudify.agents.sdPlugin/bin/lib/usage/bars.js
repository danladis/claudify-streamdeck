...
import { TEXT, TRACK, accentFor, isFailure, messageFace, messageLines, staleDot, text } from './palette.js';

const BAR = { x: 12, width: 120, height: 12, radius: 6 };

/** One row: label left, percentage right, bar beneath. */
function row(top, label, window, thresholds) {
  const known = window.usedPercent !== null;
  const percent = known ? Math.max(0, Math.min(100, Math.round(window.usedPercent))) : 0;
  const accent = known ? accentFor(statusForPercent(percent, thresholds)) : TRACK;
  const barY = top + 34;

  return (
    text(BAR.x, top + 22, label, { size: 22, fill: TEXT }) +
...
export function renderBars(snapshot, options = {}) {
  ...
  // not two empty bars implying zero usage.
  ...
  return svgDocument(
    row(8, '5H', snapshot.session, thresholds) +
      ...
      row(80, '7D', snapshot.weekly, thresholds) +
